...
class Command(BaseCommand):
    help = "Refresh FMP historical sector and industry performance for stored symbol classifications."
    ...
    def handle(self, *args, **options):
        try:
            end_date = date.fromisoformat(str(options.get("to_date") or "")) if options.get("to_date") else date.today()
            start_date = (
                date.fromisoformat(str(options.get("from_date") or ""))
        ...
        try:
            results = refresh_classification_performance(queryset.iterator(), start_date=start_date, end_date=end_date)
        except Exception as exc:
            raise CommandError(str(exc)) from exc
        observations = sum(int(item["observations_saved"]) for item in results)
        self.stdout.write(self.style.SUCCESS(f"Refreshed {len(results)} series and saved {observations} observations."))
