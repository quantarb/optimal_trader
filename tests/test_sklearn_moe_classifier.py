...
class SklearnMoERFClassifierTests(unittest.TestCase):
    ...
    def test_external_validation_frame_uses_long_positive_class(self) -> None:
        ...
        validation_frame = pd.DataFrame(
            {
                "tech_a": [0.4, 0.6, 0.7, 0.8],
                "tech_b": [0.6, 0.4, 0.3, 0.2],
                "fund_a": [12.0, 13.0, None, None],
                "fund_b": [6.0, 7.0, None, None],
                "target": [0, 1, 0, 1],
        ...
        self.assertEqual(model._train_stats["eval_mode"], "external_validation")
        self.assertEqual(model._train_stats["n_test"], len(validation_frame))
        self.assertEqual(model._class_mapping[model._classes[model._positive_class_index()]], "1")
