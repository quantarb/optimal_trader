...
def _section_event_frame(symbol_obj, section_key: str, target_index: pd.MultiIndex) -> pd.DataFrame:
    from features.section_utils import load_section_payload

    sparse = load_section_payload(symbol_obj, section_key, prefix=f"{section_key}__", filing_lag_days=0)
    if sparse.empty:
        return pd.DataFrame(columns=["date", "symbol"])
    idx = sparse.index
    return pd.DataFrame(
        {
            "date": pd.DatetimeIndex(pd.to_datetime(idx.get_level_values("date"))).normalize(),
            "symbol": idx.get_level_values("symbol").astype(str).str.upper(),
        }
    ).drop_duplicates()
