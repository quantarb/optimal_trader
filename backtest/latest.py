...
def make_autoencoder_familiarity_predictor(
    ...
    *,
    ...
    def _predict(latest_df: pd.DataFrame, ae_model: Any) -> np.ndarray:
        ...
        raise AttributeError("Autoencoder model must provide a familiarity method.")
...
def _select_primary_probability(prob_cols: list[str], latest_df: pd.DataFrame) -> np.ndarray:
    """Choose a primary classifier probability column for generic scoring."""
    if not prob_cols:
...
def score_latest_with_cluster_familiarity(
    *,
    ...
    from analysis.alpha_flavors import score_trade_flavor  # local import to keep module coupling light
