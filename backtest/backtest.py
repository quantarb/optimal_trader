...
from backtest.signals import run_signal_producers, SignalProducer
...
class ExecutionConfig(BaseModel):
    ...
    # Execution engine:
    # - "weights": weight x return engine
...
def backtest_panel(
    ...
    if str(cfg.execution_mode).lower() == "rl_env":
        ...
        from ml.rl import backtest_strategy_per_stock_discrete

        if (w < -1e-12).any().any():
            # Current RL environment implementation is long-only.
            # Use the weight x return engine for strategies with short weights.
            pass
        else:
